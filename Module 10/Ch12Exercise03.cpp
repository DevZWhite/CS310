 ...
 * Assignment: Voting Calculator (revised)
 */
...
/* Changes from Ch07Exercise04:
 *Removed fixed N_CANDIDATES constant.
 *Ask user for number of candidates.
 *Allocate candidateNames, candidateVotes, and candidatePercentages.
 *dynamically with new[].
 *Pass the candidate count (nCandidates) to all relevant functions.
 *Use pointer notation (string*, int*, double*) for arrays in function parameters.
 *Clean up memory with delete[] at the end.
*/
